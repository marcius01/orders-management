...
public class PaymentSucceededConsumer {
    ...
    @Incoming("in-payment-succeeded")
    public void consumePaymentSucceededEvent(PaymentSucceededEvent event) {
        Order order = executeChangeState(event);
        log.debug("SEND Event for order id {} to notification MS to notify user", order.getId());
    }

    @Transactional
    protected Order executeChangeState(PaymentSucceededEvent event) {
