...
import tech.skullprogrammer.orders.client.PaymentRestClient;
...
import tech.skullprogrammer.orders.model.dto.*;
...
public class ServiceOrder {
    ...
    public OrderResponseDTO virtualDeleteOrder(Long orderToDelete, Long userId, boolean admin) {
        Order order = findOrder(orderToDelete, userId, admin).orElseThrow(() -> SkullResourceException.builder().error(OrderError.ORDER_NOT_FOUND).build());
        if (order.getStatus() == EOrderStatus.CANCELED)
            throw SkullResourceException.builder().error(OrderError.ORDER_ALREADY_DELETED).build();
        order.setStatus(EOrderStatus.CANCELED);
        return mapperUtilsImpl.toOrderResponse(order);
    }

    public OrderPaymentResponseDTO getPaymentSession(Long orderId, Long userId, boolean admin) {
        Order order = findOrder(orderId, userId, admin).orElseThrow(() -> SkullResourceException.builder().error(OrderError.ORDER_NOT_FOUND).build());
        if (EOrderStatus.STOCK_RESERVED.equals(order.getStatus())) {
            PaymentRequestDTO requestDTO = PaymentRequestDTO.builder().orderId(orderId).amount(order.calculateTotalAmount()).build();
            log.info("Requesting payment for order {}", requestDTO);
            PaymentResponseDTO responseDTO = paymentRestClient.getStripePaymentUrl(requestDTO);
            log.info("Payment response {}", responseDTO);
            return OrderPaymentResponseDTO.builder().paymentURL(responseDTO.getPaymentURL()).build();
        }
        if (EOrderStatus.PAID.equals(order.getStatus())) {
            throw SkullResourceException.builder().error(OrderError.ORDER_ALREADY_PAID).build();
        }
        throw SkullResourceException.builder().error(OrderError.ORDER_NOT_PAYBLE).build();
    }


    private Optional<Order> findOrder(Long orderId, Long userId, boolean admin) {
        Order order;
        if (admin) {
            order = repositoryOrder.findById(orderId);
        } else {
            order = repositoryOrder.findByIdAndUserId(orderId, userId).orElse(null);
        }
